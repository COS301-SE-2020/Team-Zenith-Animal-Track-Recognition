...
export class TrackIdentificationsComponent implements OnInit {
			...
			.subscribe((data: any[]) => {
				...
				switch (updateOp) {
					...
					case "add":
						...
						break;
					case "delete":
						let removedRanger = this.trackIdentifications.filter(x => !newRangerList.some(y => y.rangerID == x.rangerID));
	...
	addIfNewRanger(x: any) {
		...
		for (let i = 0; i < this.trackIdentifications.length; i++)
			if (x.token == this.trackIdentifications[i].token)
				isNotNew = true;

		if (!isNotNew)
			this.trackIdentifications.push(x);
	}
	removeRanger(t: string) {
		this.trackIdentifications.splice(this.trackIdentifications.findIndex(x => x.token == t), 1);
	}
