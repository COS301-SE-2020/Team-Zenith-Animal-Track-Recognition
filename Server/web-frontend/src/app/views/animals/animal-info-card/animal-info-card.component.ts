...
export class AnimalInfoCardComponent implements OnInit {
  ...
  ngOnInit(): void {
	    this.http.get<any>('http://putch.dyndns.org:55555/graphql?query=query{animals(TokenIn:"' + JSON.parse(localStorage.getItem('currentToken'))['value'] + '"){Classification,Common_Name,Description_of_animal,Pictures{URL}}}')
      .subscribe((data: any[]) => {
        let temp = [];
